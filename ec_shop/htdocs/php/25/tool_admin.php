...
if (get_request_method() === 'POST') {
    ...
    if (get_post_route('insert') === TRUE) {
        ...
        // shop_idがあるか確認
        if (not_selected_err($shop_id)) {
          $err_msg[] = '店舗IDを入力してください';
        }
        if (large_zero_integer_check($shop_id)) {
          $err_msg[] = '店舗IDは0より大きい整数のみで入力ください';
        }
        if (count($err_msg) === 0) {
            $shop_id = (int) $shop_id;
            if (not_shop_exist($link, $shop_id)) {
                $err_msg[] = 'IDに該当する店舗がありません';
            }
        }
        // エラーなければファイルを保存
        if (count($err_msg) === 0) {
...
// 商品データ取得
$data = get_all_admin_data($link);
close_db_connect($link);

include_once '../../../include/view/view25/tool_admin.php';